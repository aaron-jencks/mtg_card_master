...
parser.add_argument("--split", "-s", help="Automatically splits the tfrecord into a train.record, and test.record", action="set_true", default=False)
...
def convert_xml_to_jpg(path: str):
	...
	xmls = map(lambda x: os.path.join(args.image_dir, x[:-4] + ".jpg"), files)
...
def remove_non_existant_files(file_list):
	...
	jpg_list = convert_xml_to_jpg(file_list)
...
def read_train_val():
	...
	remove_nonexistant_files(files)
